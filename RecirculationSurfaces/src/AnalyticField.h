...
public:
	...
	// FORWARD ONLY
	void ComputePathLineAt(const Eigen::Vector4d& WorldPosition, ofPolyline& PathLine, std::vector<double>* const times = nullptr, const bool IgnoreBounds = false, const double StepSize = 0.1, const float IntegrationTime = FLT_MAX, const unsigned int maxSteps = 500);
    ...
    Eigen::Vector3d IntegrateOverTime(const Eigen::Vector4d& WorldPosition, const double IntegrationTime, const double StepSize = 0.01, Solution3D* Solution = nullptr, const int MaxSteps = 500) const;
    ...
    {
        return IntegrateOverTime((Eigen::Vector4d() << WorldPosition(0), WorldPosition(1), WorldPosition(2), WorldPosition(3)).finished(), WorldPosition(4), StepSize, Solution);
    }
